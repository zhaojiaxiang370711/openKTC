...
- Rust `tools/devctl` detached startup falls back to local X11 display sockets like `:0` and the matching Xauthority file when the shell has no graphical environment, matching the living-room appliance use case.
...
- NAS media plan: mount `smb://192.168.0.109/nas_hdd/` to a local filesystem path and point repository media folders at that mount, so downloads and playback both use the NAS-backed files. `smb://xfn.local/nas_ssd/` is treated as an import source copied into the HDD-backed library.
