...
- 新增 Rust `tools/devctl`：
  ...
  - `dev:restart`
  - `dev:status`
  - 没有 `DISPLAY` 的 shell 中自动探测 `/tmp/.X11-unix/X0` / `X1` 和 Xauthority 文件，用于本机 appliance detached 启动。
  - `dev:status` 会在 pid 文件过期时找回真实 Electron 主进程并刷新 pid。
...
- 建立 NAS 媒体目录约定：`smb://192.168.0.109/nas_hdd/` 作为统一媒体根，`smb://xfn.local/nas_ssd/` 作为导入源复制到 HDD。
- 已将 `nas_ssd` 中可匹配的 `此生不换 (live)-黄霄雲.mp3` + `.lrc` 复制到 HDD 并导入 `My Custom Songs` 本地歌库；另外两份 `.lrc` 暂缺同名音频。
